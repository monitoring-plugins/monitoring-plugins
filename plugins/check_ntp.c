...
#define PRINTSOCKADDR(x)                                                       \
  do {                                                                         \
    printf("%u.%u.%u.%u", (x >> 24) & 0xff, (x >> 16) & 0xff, (x >> 8) & 0xff, \
           x & 0xff);                                                          \
  } while (0);

/* calculate the offset of the local clock */
static inline double calc_offset(const ntp_message *m,
...
void print_ntp_control_message(const ntp_control_message *p) {
  ...
  if (p->op & REM_RESP && p->op & OP_READSTAT) {
    ...
    for (i = 0; i < numpeers; i++) {
      printf("\tpeer id %.2x status %.2x", ntohs(peer[i].assoc),
             ntohs(peer[i].status));
...
double offset_request(const char *host, int *status) {
  ...
  while (servers_completed < num_hosts &&
         now_time - start_ts <= socket_timeout / 2) {
...
void setup_control_request(ntp_control_message *p, uint8_t opcode,
                           uint16_t seq) {
...
double jitter_request(int *status) {
  ...
  do {
    ...
    peers = tmp;
    memcpy((void *)((ptrdiff_t)peers + peer_offset), (void *)req.data,
           ntohs(req.count));
...
char *perfd_offset(double offset) {
  return fperfdata("offset", offset, "s", TRUE, offset_thresholds->warning->end,
                   TRUE, offset_thresholds->critical->end, FALSE, 0, FALSE, 0);
}
...
int main(int argc, char *argv[]) {
  ...
  if (offset_result == STATE_UNKNOWN) {
    ...
    xasprintf(&result_line, "%s %s %.10g secs", result_line, _("Offset"),
              offset);
