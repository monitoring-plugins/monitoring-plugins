...
# Required parameters
my $ssh_service = getTestParameter("MP_SSH_HOST", "A host providing SSH service", "localhost");
my $ssh_key     = getTestParameter("MP_SSH_IDENTITY", "A key allowing access to MP_SSH_HOST", "~/.ssh/id_dsa");
my $ssh_conf    = getTestParameter( "MP_SSH_CONFIGFILE", "A config file with ssh settings", "~/.ssh/config");
