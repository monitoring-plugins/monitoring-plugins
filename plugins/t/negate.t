...
# Timeout tests
$res = MPTest->testCmd( "./negate -t 2 /bin/sh -c 'sleep 5'" );
is( $res->output, 'CRITICAL - Plugin timed out after 2 seconds' );

foreach my $state (keys(%state)) {
	$res = MPTest->testCmd( "./negate -t 2 -T $state /bin/sh -c 'sleep 5'" );
...
foreach my $current_state (keys(%state)) {
	foreach my $new_state (keys(%state)) {
		$res = MPTest->testCmd( "./negate --$current_state=$new_state ./check_dummy ".$state{$current_state}." 'Fake $new_state'" );
...
foreach my $current_state (keys(%state)) {
	foreach my $new_state (keys(%state)) {
		$res = MPTest->testCmd( "./negate -s --$current_state=$new_state ./check_dummy ".$state{$current_state}." 'Fake $new_state'" );
