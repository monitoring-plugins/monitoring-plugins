...
SKIP: {
	...
	$result = MPTest->testCmd("$check_cmd -q 'SELECT 1'");
	cmp_ok($result->return_code, '==', 0, "Sqlite3 login okay and can run query");

	$result = MPTest->testCmd("$check_cmd");
	...
	$result = MPTest->testCmd("$check_cmd -m QUERY_RESULT -q 'SELECT b FROM test' -e text1");
	cmp_ok($result->return_code, '==', 0, "Query result string comparison okay");

	$result = MPTest->testCmd("$check_cmd -q 'SELECT b FROM test' -r 'eXt[0-9]'");
