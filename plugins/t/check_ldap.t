...
my $host_tcp_ldap      = getTestParameter("MP_HOST_TCP_LDAP", "A host providing the LDAP Service", "localhost");
my $ldap_base_dn       = getTestParameter("MP_LDAP_BASE_DN", "A base dn for the LDAP Service", "cn=admin");
...
SKIP: {
    skip "MP_HOST_NONRESPONSIVE not set", 2 if ! $host_nonresponsive;

    $result = MPTest->testCmd("$command -H $host_nonresponsive -b ou=blah -t 2 -w 1 -c 1");
...
SKIP: {
    skip "MP_HOSTNAME_INVALID not set", 2 if ! $hostname_invalid;

    $result = MPTest->testCmd("$command -H $hostname_invalid -b ou=blah -t 5");
...
SKIP: {
    skip "MP_HOST_TCP_LDAP not set", 12 if ! $host_tcp_ldap;
    skip "MP_LDAP_BASE_DN not set",  12 if ! $ldap_base_dn;
