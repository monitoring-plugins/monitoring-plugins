...
int np_net_ssl_init_with_hostname_version_and_cert(int sd, char *host_name, int version, char *cert, char *privkey) {
    ...
    switch (version) {
        ...
        case MP_SSLv3: /* SSLv3 protocol */
            ...
            return print_singleline_return (STATE_UNKNOWN, _("SSL protocol version 3 is not nupported by your SSL library."));
#else
...
#  ifdef USE_OPENSSL
    ...
    if (days_left > 0 && days_left <= days_till_exp_warn) {
        ...
        X509_free(certificate);
        return print_singleline_return (status, _("Certificate '%s' expires in %u %s (%s)."), cn, time_remaining, time_left >= 3600 ? "hours" : "minutes", timestamp);
    } else if (time_left < 0) {
        X509_free(certificate);
...
#  else /* ifndef USE_OPENSSL */
    return print_singleline_return (STATE_WARNING, "%s", _("Plugin does not support checking certificates."));
#  endif /* USE_OPENSSL */
}
