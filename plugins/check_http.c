...
char *randbuff;
X509 *server_cert;
#define my_recv(buf, len)                                                      \
  ((use_ssl) ? np_net_ssl_read(buf, len) : read(sd, buf, len))
#define my_send(buf, len)                                                      \
  ((use_ssl) ? np_net_ssl_write(buf, len) : send(sd, buf, len, 0))
...
int main(int argc, char **argv) {
  ...
  /* Set default URL. Must be malloced for subsequent realloc if
   * --onredirect=follow */
  ...
  while (1) {
    ...
    case 'k': /* Additional headers */
      if (http_opt_headers_count == 0)
        ...
        http_opt_headers = realloc(http_opt_headers,
    ...
    case 'C': /* Check SSL cert validity */
      ...
      goto enable_ssl;
#endif
    case CONTINUE_AFTER_CHECK_CERT: /* don't stop after the certificate is
                                       checked */
#ifdef HAVE_SSL
...
#ifdef HAVE_SSL
    enable_ssl:
      /* ssl_version initialized to 0 as a default. Only set if it's non-zero.
         This helps when we include multiple parameters, like -S and -C
         combinations */
      use_ssl = true;
      if (c == 'S' && optarg != NULL) {
    ...
    case 'b': /* proxy-authorization info */
      ...
      break;
    case 'P': /* HTTP POST data in URL encoded format; ignored if settings
                 already */
      if (!http_post_data)
...
static time_t parse_time_string(const char *string) {
  ...
  if (isupper(string[0]) && /* Tue */
      islower(string[1]) && islower(string[2]) && ',' == string[3] &&
      ' ' == string[4] && (isdigit(string[5]) || string[5] == ' ') && /* 25 */
      isdigit(string[6]) && ' ' == string[7] && isupper(string[8]) && /* Dec */
      islower(string[9]) && islower(string[10]) && ' ' == string[11] &&
      isdigit(string[12]) && /* 2001 */
      isdigit(string[13]) && isdigit(string[14]) && isdigit(string[15]) &&
      ' ' == string[16] && isdigit(string[17]) && /* 02: */
      isdigit(string[18]) && ':' == string[19] &&
      isdigit(string[20]) && /* 59: */
      isdigit(string[21]) && ':' == string[22] &&
    ...
    tm.tm_mday =
        10 * (string[5] == ' ' ? 0 : string[5] - '0') + (string[6] - '0');
...
static int check_document_dates(const char *headers, char **msg) {
  ...
  if (!server_date || !*server_date) {
    ...
    if (srv_data <= 0) {
      xasprintf(msg, _("%sServer date \"%100s\" unparsable, "), *msg,
                server_date);
      date_result = max_state_alt(STATE_CRITICAL, date_result);
    } else if (doc_data <= 0) {
      xasprintf(msg, _("%sDocument date \"%100s\" unparsable, "), *msg,
                document_date);
      date_result = max_state_alt(STATE_CRITICAL, date_result);
      ...
      if (n > (60 * 60 * 24 * 2)) {
        ...
        xasprintf(msg, _("%sLast modified %d:%02d:%02d ago, "), *msg,
...
int check_http(void) {
  ...
  if (server_address != NULL && strcmp(http_method, "CONNECT") == 0 &&
    ...
    if (http_opt_headers_count) {
      ...
      /* This cannot be free'd here because a redirection will then try to
       * access this and segfault */
  ...
  if (use_ssl == true) {
    ...
    result = np_net_ssl_init_with_hostname_version_and_cert(
        sd, (use_sni ? host_name : NULL), ssl_version, client_cert,
        client_privkey);
    if (verbose)
  ...
  if (verbose)
    printf("%s://%s:%d%s is %d characters\n", use_ssl ? "https" : "http",
           server_address, server_port, server_url, (int)pagesize);
  ...
  /* Bypass normal status line check if server_expect was set by user and not
   * default */
  ...
  /* reset the alarm - must be called *after* redir or we'll never die on
   * redirects! */
  alarm(0);
  ...
  if (show_extended_perfdata)
    xasprintf(
        ...
        msg, page_len, elapsed_time, (display_html ? "</A>" : ""),
    ...
    xasprintf(&msg, _("%s - %d bytes in %.3f second response time %s|%s %s"),
              msg, page_len, elapsed_time, (display_html ? "</A>" : ""),
              perfd_time(elapsed_time), perfd_size(page_len));
...
/* relative reference redirect like //www.site.org/test
...
void redir(char *pos, char *status_line) {
  ...
  while (pos) {
    sscanf(pos, "%1[Ll]%*1[Oo]%*1[Cc]%*1[Aa]%*1[Tt]%*1[Ii]%*1[Oo]%*1[Nn]:%n",
           xx, &i);
    if (i == 0) {
  ...
  if (++redir_depth > max_depth)
    die(STATE_WARNING,
        _("HTTP WARNING - maximum redirection depth %d exceeded - "
          "%s://%s:%d%s%s\n"),
  ...
  if (server_port == i && !strncmp(server_address, addr, MAX_IPV4_HOSTLENGTH) &&
    ...
    die(STATE_CRITICAL,
        _("HTTP CRITICAL - redirection creates an infinite loop - "
          "%s://%s:%d%s%s\n"),
...
char *perfd_time(double elapsed_time) {
  return fperfdata("time", elapsed_time, "s", thlds->warning ? true : false,
                   thlds->warning ? thlds->warning->end : 0,
                   thlds->critical ? true : false,
                   thlds->critical ? thlds->critical->end : 0, true, 0, true,
                   socket_timeout);
}

char *perfd_time_connect(double elapsed_time_connect) {
  return fperfdata("time_connect", elapsed_time_connect, "s", false, 0, false,
                   0, false, 0, true, socket_timeout);
}

char *perfd_time_ssl(double elapsed_time_ssl) {
  return fperfdata("time_ssl", elapsed_time_ssl, "s", false, 0, false, 0, false,
                   0, true, socket_timeout);
}

char *perfd_time_headers(double elapsed_time_headers) {
  return fperfdata("time_headers", elapsed_time_headers, "s", false, 0, false,
                   0, false, 0, true, socket_timeout);
}

char *perfd_time_firstbyte(double elapsed_time_firstbyte) {
  return fperfdata("time_firstbyte", elapsed_time_firstbyte, "s", false, 0,
                   false, 0, false, 0, true, socket_timeout);
}

char *perfd_time_transfer(double elapsed_time_transfer) {
  return fperfdata("time_transfer", elapsed_time_transfer, "s", false, 0, false,
                   0, false, 0, true, socket_timeout);
}

char *perfd_size(int page_len) {
  return perfdata("size", page_len, "B", (min_page_len > 0 ? true : false),
                  min_page_len, (min_page_len > 0 ? true : false), 0, true, 0,
                  false, 0);
}
...
void print_help(void) {
  ...
  printf(_("In the second form, connect to the server and check the TLS "
           "certificate."));
#endif
  ...
  printf(" %s\n", "-j, --method=STRING  (for example: HEAD, OPTIONS, TRACE, "
                  "PUT, DELETE, CONNECT, CONNECT:POST)");
  printf("    %s\n", _("Set HTTP method."));
  printf(" %s\n", "-N, --no-body");
  printf("    %s\n",
  ...
  printf(" %s\n", _("Successful connects return STATE_OK, refusals and "
                    "timeouts return STATE_CRITICAL"));
  ...
  printf(" %s\n",
         _("check_http -I 192.168.100.35 -p 80 -u https://www.verisign.com/ -S "
           "-j CONNECT -H www.verisign.com "));
  printf(" %s\n", _("all these options are needed: -I <proxy> -p <proxy-port> "
                    "-u <check-url> -S(sl) -j CONNECT -H <webserver>"));
  printf(" %s\n", _("a STATE_OK will be returned. When the server returns its "
                    "content but exceeds"));
  printf(" %s\n", _("the 5-second threshold, a STATE_WARNING will be returned. "
                    "When an error occurs,"));
  printf(" %s\n", _("a STATE_CRITICAL will be returned. By adding a colon to "
                    "the method you can set the method used"));
...
void print_usage(void) {
  ...
  printf("       [-b proxy_auth] [-f "
         "<ok|warning|critcal|follow|sticky|stickyport>]\n");
  printf("       [-e <expect>] [-d string] [-s string] [-l] [-r <regex> | -R "
         "<case-insensitive regex>]\n");
  printf("       [-P string] [-m <min_pg_size>:<max_pg_size>] [-4|-6] [-N] [-M "
         "<age>]\n");
