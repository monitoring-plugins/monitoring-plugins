 ...
 * CHECK_POP.C
 *
 * Program: POP3 plugin for Nagios
 * License: GPL
 ...
 * Description:
 *
 * This plugin will attempt to open an POP connection with the host.
...
#define PROGNAME "check_pop"

#define POP_PORT	110
#define POP_EXPECT     "+OK"
#define POP_QUIT	"QUIT\n"
	...
	if (result == STATE_OK) {
		...
		if (result == -1) {
			...
			if (!strstr (buffer, server_expect)) {
				if (server_port == POP_PORT)
					printf ("Invalid POP response received from host\n");
				else
					printf ("Invalid POP response received from host on port %d\n",
									server_port);
				result = STATE_WARNING;
			}
				...
				if (verbose == TRUE)
					printf ("POP %s - %d sec. response time, %s\n",
									(result == STATE_OK) ? "ok" : "problem",
									(int) (end_time - start_time), buffer);
				else
					printf ("POP %s - %d second response time\n",
									(result == STATE_OK) ? "ok" : "problem",
		...
		send (sd, POP_QUIT, strlen (POP_QUIT), 0);
		close (sd);
	}
...
process_arguments (int argc, char **argv)
{
	...
	if (server_expect == NULL)
		server_expect = strscpy (NULL, POP_EXPECT);
...
print_help (void)
{
	...
	printf
		("\nOptions:\n"
		 ...
		 POP_PORT, POP_EXPECT, DEFAULT_SOCKET_TIMEOUT);
	support ();
}
